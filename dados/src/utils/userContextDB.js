...
class UserContextDB {
  ...
  /**
   * Adiciona uma observação à memória social
   */
  addSocialObservation(userId, observacao) {
    const context = this.getUserContext(userId);
    ...
    if (!context.memoria_social) {
      context.memoria_social = {
        observacoes: [],
        pessoas_mencionadas: [],
        assuntos_recorrentes: []
      };
    ...
    if (!context.memoria_social.observacoes.includes(texto)) {
      context.memoria_social.observacoes.push(texto);
    }

    if (context.memoria_social.observacoes.length > 30) {
      context.memoria_social.observacoes =
        context.memoria_social.observacoes.slice(-30);
    }
  ...
  /**
   * Registra uma pessoa mencionada em conversa
   */
  addSocialPerson(userId, pessoa) {
    const context = this.getUserContext(userId);

    if (!pessoa || typeof pessoa !== 'string') return false;
    ...
    if (!context.memoria_social.pessoas_mencionadas.includes(nome)) {
      context.memoria_social.pessoas_mencionadas.push(nome);
    }

    if (context.memoria_social.pessoas_mencionadas.length > 30) {
      context.memoria_social.pessoas_mencionadas =
        context.memoria_social.pessoas_mencionadas.slice(-30);
    }
  ...
  /**
   * Registra um assunto recorrente
   */
  addSocialTopic(userId, assunto) {
    const context = this.getUserContext(userId);

    if (!assunto || typeof assunto !== 'string') return false;

    const texto = assunto.trim();
    if (!texto) return false;
    ...
    if (!context.memoria_social.assuntos_recorrentes.includes(texto)) {
      context.memoria_social.assuntos_recorrentes.push(texto);
    }

    if (context.memoria_social.assuntos_recorrentes.length > 30) {
      context.memoria_social.assuntos_recorrentes =
        context.memoria_social.assuntos_recorrentes.slice(-30);
    }
  ...
  /**
   * Obtém somente a memória social
   */
  getSocialMemory(userId) {
    const context = this.getUserContext(userId);
    ...
    return {
      observacoes: context.memoria_social.observacoes.slice(-10),
      pessoas_mencionadas: context.memoria_social.pessoas_mencionadas.slice(-10),
      assuntos_recorrentes: context.memoria_social.assuntos_recorrentes.slice(-10)
    };
  }
